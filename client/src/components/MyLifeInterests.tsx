...
export function MyLifeInterests({ interests }: { interests: string[] }) {
  ...
  return (
    <div style={{ display: "flex", flexWrap: "wrap", gap: 8, alignItems: "center" }}>
      {interests.map((i) => (
        <span key={i} style={{ background: colors.greenBg, color: colors.greenText, borderRadius: 999, padding: "7px 14px", fontSize: 13, fontWeight: 600 }}>
          {i}
        </span>
      ))}
      <button
        onClick={() => navigate("/onboarding")}
        style={{ background: "none", border: `1px solid ${colors.borderStrong}`, borderRadius: 999, padding: "7px 14px", fontSize: 13, fontWeight: 600, color: colors.text, cursor: "pointer" }}
      >
        + Add interest
      </button>
    </div>
